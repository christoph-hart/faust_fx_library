...
#include "src/MonoFlanger.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct MonoFlangerMetaData {
		SN_NODE_ID("MonoFlanger");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using MonoFlanger = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _MonoFlanger, MonoFlangerMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
