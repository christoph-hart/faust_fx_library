...
#include "src/PhaserStereo.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct PhaserStereoMetaData {
		SN_NODE_ID("PhaserStereo");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using PhaserStereo = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _PhaserStereo, PhaserStereoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
