...
#include "src/zita_reverb.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct zita_reverbMetaData {
		SN_NODE_ID("zita_reverb");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using zita_reverb = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _zita_reverb, zita_reverbMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
