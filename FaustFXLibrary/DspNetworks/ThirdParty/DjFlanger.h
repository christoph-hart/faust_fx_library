...
#include "src/DjFlanger.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct DjFlangerMetaData {
		SN_NODE_ID("DjFlanger");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using DjFlanger = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _DjFlanger, DjFlangerMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
