...
struct _ChaosFlanger final : public ::faust::dsp {
	...
	void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("Feedback", &fHslider4, FAUSTFLOAT(0.75f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.001f));
		ui_interface->addHorizontalSlider("FlangeLeft", &fHslider2, FAUSTFLOAT(5e+01f), FAUSTFLOAT(0.0f), FAUSTFLOAT(3.5e+02f), FAUSTFLOAT(0.01f));
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow2 = std::max<float>(0.0f, std::min<float>(3.5e+02f, float(fHslider2)));
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec2[0] = float(1 - iVec0[1]) + fSlow5 * fRec2[1] - fSlow4 * fRec1[1];
			float fTemp0 = fConst0 * (0.005f * (fRec1[0] + 1.0f) + 0.001f);
			float fTemp1 = fSlow2 + fTemp0;
			float fTemp2 = std::floor(fTemp1);
			float fTemp3 = float(input0[i0]);
			float fTemp4 = fSlow6 * fRec0[1] - fTemp3;
			fVec1[IOTA0 & 2047] = fTemp4;
			int iTemp5 = int(fTemp1);
			fRec0[0] = fVec1[(IOTA0 - std::min<int>(1049, std::max<int>(0, iTemp5 + 1))) & 2047] * (fSlow2 + (fTemp0 - fTemp2)) + fVec1[(IOTA0 - std::min<int>(1049, std::max<int>(0, iTemp5))) & 2047] * (fTemp2 + (1.0f - fTemp0) - fSlow2);
			output0[i0] = FAUSTFLOAT(0.5f * (fTemp3 + fRec0[0] * fSlow1));
			float fTemp6 = fConst0 * (0.005f * (fRec2[0] + 1.0f) + 0.001f);
			float fTemp7 = fSlow7 + fTemp6;
			float fTemp8 = std::floor(fTemp7);
			float fTemp9 = float(input1[i0]);
			float fTemp10 = fSlow6 * fRec3[1] - fTemp9;
			fVec2[IOTA0 & 2047] = fTemp10;
			int iTemp11 = int(fTemp7);
			fRec3[0] = (fSlow7 + (fTemp6 - fTemp8)) * fVec2[(IOTA0 - std::min<int>(1049, std::max<int>(0, iTemp11 + 1))) & 2047] + fVec2[(IOTA0 - std::min<int>(1049, std::max<int>(0, iTemp11))) & 2047] * (fTemp8 + (1.0f - fTemp6) - fSlow7);
			output1[i0] = FAUSTFLOAT(0.5f * (fTemp9 + fRec3[0] * fSlow1));
			iVec0[1] = iVec0[0];
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
			IOTA0 = IOTA0 + 1;
	...
	FAUST_ADDHORIZONTALSLIDER("Feedback", fHslider4, 0.75f, 0.0f, 1.0f, 0.001f);
	FAUST_ADDHORIZONTALSLIDER("FlangeLeft", fHslider2, 5e+01f, 0.0f, 3.5e+02f, 0.01f);
	...
	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, Feedback, "Feedback", fHslider4, 0.75f, 0.0f, 1.0f, 0.001f) \
		p(HORIZONTALSLIDER, FlangeLeft, "FlangeLeft", fHslider2, 5e+01f, 0.0f, 3.5e+02f, 0.01f) \
