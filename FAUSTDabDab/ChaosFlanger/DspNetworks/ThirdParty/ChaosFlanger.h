...
#include "src/ChaosFlanger.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct ChaosFlangerMetaData {
		SN_NODE_ID("ChaosFlanger");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using ChaosFlanger = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _ChaosFlanger, ChaosFlangerMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
